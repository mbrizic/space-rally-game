export type Surface = {
  name: "tarmac" | "gravel" | "sand" | "ice" | "offtrack";
  frictionMu: number;
...
function surfaceWeightsForTheme(themeKind?: StageThemeKind): { tarmac: number; gravel: number; ice: number } {
  switch (themeKind) {
    case "desert":
      // Desert: loose surface is sand (not gravel). We generate "gravel" and map it to sand.
      return { tarmac: 0.28, gravel: 0.72, ice: 0.0 };
    case "arctic":
      return { tarmac: 0.22, gravel: 0.40, ice: 0.38 };
    case "rainforest":
      // Wet, earthy, but never icy.
      return { tarmac: 0.34, gravel: 0.66, ice: 0.0 };
    case "temperate":
    default:
      // Rally-friendly default: non-tarmac is common, but allow occasional ice patches.
      return { tarmac: 0.40, gravel: 0.50, ice: 0.10 };
  }
}
...
export function surfaceForTrackSM(
  ...
  const totalW0 = Math.max(1e-6, tarmacWeight + gravelWeight + iceWeight);
  ...
  if (tW > tarmacMax) {
    const other = Math.max(1e-6, gW + iW);
  ...
  const forcedVarietySurface = (() => {
    const nonTarmacW = Math.max(0, gW + (iW > 1e-6 ? iW : 0));
  ...
  const expectedRunLenSeg = (name: "tarmac" | "gravel" | "ice"): number => {
    switch (name) {
      case "tarmac":
        return 3.0;
      case "ice":
  ...
  const pickSurfaceForRun = (runIdx: number): "tarmac" | "gravel" | "ice" => {
    ...
    const qSum = Math.max(1e-6, qt + qg + qi);
  ...
  const runLenForSurface = (name: "tarmac" | "gravel" | "ice", runIdx: number): number => {
  ...
  const surfacesBySegment: ("tarmac" | "gravel" | "ice")[] = new Array(numSegments);
  let seg = 0;
  let runIdx = 0;
  ...
  let surfaceName: "tarmac" | "gravel" | "ice" = (surfacesBySegment[segmentIdx] ?? "tarmac");
  ...
  switch (surfaceName) {
    ...
    case "gravel":
      if (themeKind === "desert") {
        // Sand: a bit lower grip and more drag than gravel.
        return { name: "sand", frictionMu: 0.84 + surfaceRand(seed + segmentIdx) * 0.06, rollingResistanceN: 520 + surfaceRand(seed * 1.7 + segmentIdx) * 60 };
      }
